...
class ZoomView(context: Context, attributes: AttributeSet? = null) : FrameLayout(context, attributes) {
    ...
    internal var zoomY: Float = 0f
    internal var smoothZoomX: Float = 0f
    internal var smoothZoomY: Float = 0f
    ...
    private fun processSingleTouchOutsideMinimap(ev: MotionEvent) {
        ...
        val distanceFromTouchStart = Math.hypot(lx.toDouble(), ly.toDouble()).toFloat()
        var dx = x - touchLastX
        var dy = y - touchLastY
        touchLastX = x
        touchLastY = y
    ...
    override fun dispatchDraw(canvas: Canvas) {
        ...
        for (i in (0 until childCount)) {
            ...
            val cm = Matrix(m)
            cm.preTranslate(child.left.toFloat(), child.top.toFloat())
